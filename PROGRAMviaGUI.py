...
class Main(tk.Frame):
    ...
    def cases(self,a, b, c, k): # проверка на особые случаи
        ...
        if a == b == 0:
            ...
            self.main(a,b,c)
            x = 'x = '+str(self.x0) + self.sign(b) + str(abs(b)) + '*k;'
            y = 'y = '+str(self.y0) + self.sign(-a) + str(abs(a)) + '*k'
            xr = self.x0 + b*k
            yr = self.y0 - a*k
    ...
    def main(self, a,b,c): # расширенный алгоритм Евклида
        ...
        while bt:
            ...
            x0, xr0 = xr0, x0 - xr0*q
            y0, yr0 = yr0, y0 - yr0*q
        self.x0, self.y0, a, b = x0*(c/self.g), y0*(c/self.g), a/self.g, b/self.g
    ...
    def start1(self):
        try:
            ...
            if not self.check_for_zeros:
                self.outputX.configure(text=round(self.xr))
                self.outputY.configure(text=round(self.yr))
            else:
